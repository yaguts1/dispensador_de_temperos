...
#define USE_INSECURE_TLS     1      // DEV: 1, Produção: 0
#define DEBUG_HTTP           1
...
// ============================================================================
// GLOBAIS - JOBS (compartilhados com job_execution.ino)
// ============================================================================
...
// ============================================================================
// FORWARD DECLARATIONS (job_execution.ino)
// ============================================================================
...
input{width:100%;padding:10px;border-radius:10px;border:1px solid #31407a;background:#0f1733;color:#fff;box-sizing:border-box}
...
fetch('/info').then(r=>r.json()).then(j=>{
 document.getElementById('info').textContent =
  `UID: ${j.uid} • MAC: ${j.mac} • FW: ${j.fw} • RSSI: ${j.rssi}dBm • IP: ${j.ip}`;
...
bool isSixDigitsAllowEmpty(const String& s) {
...
void httpApplyCommon(HTTPClient& http, const String& path) {
...
int httpPOST(const String& path, const String &jsonBody, String &out) {
  ...
  Serial.printf("[HTTP POST] %s://%s:%u%s\n", ep.https ? "https" : "http",
                ep.host.c_str(), ep.port, path.c_str());
  Serial.printf("[HTTP POST] Body (len=%d)\n", jsonBody.length());
...
// ============================================================================
// DEVICE / CLAIM / HEARTBEAT
// ============================================================================

String _json_pick_token(const JsonVariantConst& root) {
...
String _scan_token_from_body(const String& body) {
...
bool doClaim() {
  if (!st_claim.length()) { 
    Serial.println("[CLAIM] Sem claim_code."); 
    return false; 
  }
  ...
  if (!tok.length()) { 
    Serial.println("[CLAIM] token ausente."); 
    return false; 
  }
...
bool pollNextJob() {
  ...
  if (code <= 0) {
    fails++; 
    Serial.printf("[POLL] erro %d (%d fails)\n", code, fails);
  ...
  if (code == 204) { 
    Serial.println("[POLL] 204 (sem job)"); 
    fails = 0; 
    return false; 
  }
  ...
  if (code == 200 && out.length() > 0) {
    ...
    // Salvar em Flash ANTES de executar
    ...
    if (!deserializeJson(doc, out)) {
      ...
      g_currentJob.logPayload[0] = '\0';
      
      saveJob(g_currentJob);
      Serial.printf("[POLL] Job %d salvo em Flash\n", g_currentJob.jobId);
    }
    ...
    if (!execOk || !reportOk) {
      Serial.printf("[POLL] Exec:%d Report:%d (vai retry)\n", execOk, reportOk);
    }
...
#if DEBUG_HTTP
  Serial.printf("[PREFS] api='%s' token_len=%d claim=%s\n",
...
void setup() {
  Serial.begin(115200); 
  delay(200);
  Serial.println("\n=== Yaguts Dispenser (ESP32) v" FW_VERSION " ===");
  Serial.printf("UID: %s  MAC: %s\n", chipUID().c_str(), macAddr().c_str());

  setupPins();
  loadPrefs();

  // Tentar retomar job anterior
  if (tryResumeJobFromFlash()) {
    Serial.println("[SETUP] Job anterior detectado em Flash");
  }
...
void loop() {
  ...
  switch (state) {
    ...
    case ST_ONLINE:
      ...
      // Retry de report se job pendente
      if (g_currentJob.jobId && now - g_lastReportAttempt >= REPORT_RETRY_INTERVAL) {
        Serial.println("[LOOP] Retry report job pendente...");
        reportJobCompletion();
